...
- Broken skill mid-task → disclose the problem; use reflect to propose a generic fix. Keep it local unless publishing a change was requested.
- Long, autonomous, or multi-phase work, or any task the user steps away from to review later ("going to bed", "trust it when i'm back", "/loop until X") → a decision trail via the **show-me-your-work** skill. Keep it under STATE_DIR; never commit workflow notes to the target repository.
...
Follow the local runtime policy and the user's authorized scope. Continue reversible local work without redundant permission questions. External actions require task authorization. Opening a PR is optional.
...
- **Opening a PR.** Invoke only when opening a PR is part of the user request. `playbooks/opening-a-pr.md`.
