...
2. **Install the runtime.** Run `orch --store <STATE_DIR>/orchestrate init` and pass that --store path on every subsequent command. Open the trail via the show-me-your-work skill, write the standing orders before any spawn, and seed `frontier.json` from existing PRs with `orch frontier set --repo <repo-dir>`.
