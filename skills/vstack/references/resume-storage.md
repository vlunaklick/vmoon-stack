...
Pause and pickup share one project locator. The `resume.mjs` command under `skills/vstack/scripts/` resolves the project's Git common directory and uses that identity to store checkpoints under `STATE_DIR/resume/` outside the repository. `latest.json` points to the latest complete checkpoint and records the worktree, branch, timestamp, note, and requested artifacts with content hashes.
