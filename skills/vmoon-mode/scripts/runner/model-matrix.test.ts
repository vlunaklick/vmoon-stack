...
const DISPATCH_PATH = join(
  PLUGIN_ROOT,
  "skills/vmoon-mode/references/provider-dispatch.md"
);
const SETUP_PATH = join(PLUGIN_ROOT, "skills/setup-vstack/SKILL.md");
const AGENTS_DIR = join(PLUGIN_ROOT, "agents");

const MATRIX_HEADER = [
  "Family",
  "Upstream vstack choice",
  "Provider",
  "Model",
  "Default effort",
  "Selectable efforts",
...
function firstRunSheet(setup: string): string {
  ...
  if (!match) {
    throw new Error("setup-vstack is missing the first-run sheet fence");
  }
  return match[1];
}
...
describe("model matrix", () => {
  ...
  it("ships exactly the declared Claude-native frontier agents", () => {
    ...
    for (const row of rows) {
      ...
      for (const effort of row.selectableEfforts) {
        const name = `vstack-${stem}-${effort}`;
        expected.add(`${name}.md`);
        const text = readFileSync(join(AGENTS_DIR, `${name}.md`), "utf8");
        const { fields, body } = parseFrontmatter(text);
        expect(fields).toEqual({
          name,
          description: `Native Claude lane for vstack roles configured as ${row.provider}:${row.model}@${effort}.`,
          model: row.model,
          effort,
          background: "true",
    ...
    const shipped = readdirSync(AGENTS_DIR)
      .filter((name) => name.startsWith("vstack-") && name.endsWith(".md"))
      .sort();
  ...
  it("binds Claude-native dispatch to the matrix mapping", () => {
    ...
    expect(nativeLanes).toContain("`vstack-<stem>-<effort>`");
  });
  ...
  it("normalizes old rolling-family pins before any runtime route", () => {
    ...
    expect(normalization).toContain("`/setup-vstack` will rewrite it");
